...
const counterServiceOfInterestUuid = '00000001-0002-0003-0004-000000004000' 					//uuid of button service
...
mqttClient.on('message', messageEventHandler);
	...
	console.log("services are " + services)
  	...
  	if (services.includes(buttonServiceOfInterestUuid)) {
  		...
  		const primaryButtonService = await gattServer.getPrimaryService(buttonServiceOfInterestUuid)	
	 	buttonAChar = await primaryButtonService.getCharacteristic(buttonACharacteristicOfInterestUuid)  	//uuid of read/write characteristic of button service
	 	A_button_val = await buttonAChar.readValue()
	 	console.log(A_button_val)
	}
  	if (services.includes(counterServiceOfInterestUuid)) {
  		console.log('got the Counter service')
  		const primaryCounterService = await gattServer.getPrimaryService(counterServiceOfInterestUuid)	
	 	unlockCounterChar = await primaryCounterService.getCharacteristic(unlockCounterCharacteristicOfInterestUuid)  	//uuid of read/write characteristic of button service
	 	unlock_counter_val = await unlockCounterChar.readValue()
	 	console.log(unlock_counter_val)
	 	//await unlockCounterChar.startNotifications()
	 	//unlockCounterChar.on('valuechanged', buffer => {
		//	console.log(buffer) })
